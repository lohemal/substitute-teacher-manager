...
fn real_db_전담시간_확인안내() {
    ...
    for day in 1..=5 {
        ...
        for (class_id, period, sub_teacher) in covered.iter().take(3) {
            ...
            // 그 반 담임은 이 시간 '수업 중'이 아니어야 한다 — 전담이 대신 들어가므로.
            //
            // 실제 DB에는 그 담임이 이미 다른 보결을 맡고 있거나, 그날 결근으로
            // 등록되어 있을 수 있다. 그것들은 정상적인 제외 사유이므로 여기서
            // 확인하려는 것은 하나뿐이다 — **자기 반 수업으로 잡히지 않는다.**
            let (ok, code) = ctx.verdict(&r2, hr);
            assert!(
                ok || code != "EXCLUDED_REGULAR_CLASS",
                "담임 {} 은 전담 시간에 공강이므로 '수업 중'으로 빠지면 안 된다 ({code})",
                ctx.name_of(hr)
            );
